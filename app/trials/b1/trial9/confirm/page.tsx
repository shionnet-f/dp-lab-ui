...
type ShippingKey = "standard" | "express" | "scheduled";
...
function getShippingPriceTable() {
  const sorted = [...products6].sort((a, b) => a.priceYen - b.priceYen);
  const cheapest = sorted[0];
  const minPrice = cheapest.priceYen;
  const maxGap = Math.max(
    ...sorted.map((product) => product.priceYen - minPrice),
  );

  const table: Record<string, Record<ShippingKey, number>> = {};

  sorted.forEach((product, index) => {
    if (index === 0) {
      table[product.id] = {
        standard: maxGap + 1400,
        express: maxGap + 1700,
        scheduled: maxGap + 1600,
      };
      return;
    }

    table[product.id] = {
      standard: 250 + (index - 1) * 80,
      express: 550 + (index - 1) * 80,
      scheduled: 450 + (index - 1) * 80,
    };
  });
...
function getShippingInfo(productId: string, shipping?: string) {
  const table = getShippingPriceTable();
  const prices = table[productId] ?? table[products6[0].id];

  switch (shipping) {
    case "express":
      return { label: "お急ぎ便", priceYen: prices.express };
    case "scheduled":
      return { label: "当日便", priceYen: prices.scheduled };
    case "standard":
      return { label: "通常配送", priceYen: prices.standard };
    default:
...
export default async function ConfirmPageB1Trial9({ searchParams }: Props) {
  ...
  const shippingInfo = getShippingInfo(selectedProduct.id, shipping);
          ...
          <section className="grid h-full grid-cols-[1.5fr_1fr] gap-6">
            ...
            <div className="grid h-full grid-rows-[1fr_auto] overflow-hidden rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="grid gap-3 pt-6">
                <Link
                  href={`/trials/b1/trial9/complete?${completeParams.toString()}`}
                ...
                <Link
                  href={`/trials/b1/trial9/checkout?${backParams.toString()}`}
