  ...
  return (
    <section className="h-[145px] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <div className="h-[70px] px-5">
        {shippingMethod ? (
          <div className="flex h-full items-center justify-between border border-gray-200 px-4 text-[18px] font-bold text-gray-800">
            <span className="truncate pr-4">{shippingMethod.name}</span>
            <span className="shrink-0">¥{yen(shippingMethod.priceYen)}</span>
          </div>
        ) : (
          <div className="flex h-full items-center border border-gray-200 px-4 text-[18px] font-bold text-gray-500">
            未選択
          </div>
        )}
      </div>
