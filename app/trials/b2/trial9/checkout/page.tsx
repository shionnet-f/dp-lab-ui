...
import { getProductById, getShippingMethodsForProduct, trial9Data } from "../data";
...
export default function CheckoutPageB2Trial9({ searchParams }: Props) {
  ...
  const shippingMethods = useMemo(() => getShippingMethodsForProduct(selectedProduct.id), [selectedProduct.id]);
  const defaultShipping = shippingMethods[0]?.id ?? "";
  const [shipping, setShipping] = useState<string>(sp?.shipping ?? defaultShipping);
  ...
  return (
    <main className="h-screen overflow-hidden bg-gray-50 px-8 py-8">
      <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        <form
          action="/trials/b2/trial9/confirm"
          method="GET"
