...
import { getOptionsByIds, getProductById, getShippingById, trial8Data } from "../data";
...
export default async function ConfirmPageB2Trial8({ searchParams }: Props) {
  ...
  const isSubscriptionProduct = selectedProduct.id === "b2-t7-p3";
        ...
        <div className="grid flex-1 grid-rows-[152px_228px_176px_120px_88px] gap-6">
              ...
              <div className="min-w-0">
                ...
                {isSubscriptionProduct ? (
                  <div className="mt-2 space-y-1 text-sm text-gray-500">
                    <p>※ この価格は初回お届け分の定期便価格です。</p>
                    <p>※ 2回目以降も毎月同じ商品が継続して届きます。</p>
                  </div>
                ) : (
            ...
            <div className="flex h-full flex-col justify-between">
              <div className="space-y-2 text-sm text-gray-900">
                <div className="flex items-center justify-between">
                  <span>{isSubscriptionProduct ? "初回価格" : "商品価格"}</span>
          ...
          <section className="rounded-xl border border-gray-200 bg-white px-6 py-5 shadow-sm">
            <div className="flex h-full flex-col gap-3">
              <Link
                href={`/trials/b2/trial8/complete?${completeParams.toString()}`}
              ...
              <Link
                href={`/trials/b2/trial8/checkout?${backParams.toString()}`}
