...
const confirmPath = getTrialPath("b2", "trial10", "confirm");
const productPath = getTrialPath("b2", "trial10", "product");
...
export default function CheckoutPageB2Trial10({ searchParams }: Props) {
  ...
  const selectedShippingMethod = trial10Data.shippingMethods.find((method) => method.id === shipping) ?? null;
  const selectedOptionItems = trial10Data.options.filter((option) => options.includes(option.id));
  ...
  return (
    <main className="h-[1080px] w-[1920px] overflow-hidden bg-gray-50">
      <div className="h-full w-full">
        <TrialPageHeader purchaseConditions={trial10Data.purchaseConditions} title="購入手続き" />
        <form onSubmit={async (e) => {
          ...
          <section className="grid h-[438px] w-[1160px] grid-cols-[550px_550px] gap-[60px] overflow-hidden">
            <ShippingMethodSection shippingMethods={trial10Data.shippingMethods} selectedShipping={shipping} onChangeShipping={setShipping} set={set} trial={trial} />
            <OptionSection options={trial10Data.options} selectedOptions={options} onToggleOption={toggleOption} set={set} trial={trial} />
          </section>
          <div className="h-[60px]" />
