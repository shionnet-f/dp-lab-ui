...
const checkoutPath = getTrialPath("b2", "trial2", "checkout");

type ProductForB2Trial2 = (typeof trial2Data.products)[number];
...
type B2Trial2ProductCardProps = {
  product: ProductForB2Trial2;
  set: string;
  trial: string;
  checkoutPath: string;
};

function B2Trial2ProductCard({
  product,
  set,
  trial,
  checkoutPath,
}: B2Trial2ProductCardProps) {
  ...
  async function handleSelect(source: "card" | "detail_modal") {
    ...
    <article className="h-full w-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="grid h-full grid-cols-[520px_280px_360px]">
        ...
        <div className="flex h-full min-w-0 items-center justify-center px-[20px]">
          <div className="h-[50px] w-full min-w-0 overflow-hidden" aria-hidden="true" />
        </div>

        {/* 右側：ボタン2つ */}
        <div className="flex h-full items-center justify-center">
          <div className="flex items-center gap-[60px]">
            <B2Trial2ProductDetailDialog
              product={product}
              set={set}
              trial={trial}
              checkoutPath={checkoutPath}
              onSelect={() => handleSelect("detail_modal")}
            />
...
type B2Trial2ProductDetailDialogProps = {
  product: ProductForB2Trial2;
  set: string;
  trial: string;
  checkoutPath: string;
...
function B2Trial2ProductDetailDialog({
  product,
  set,
  trial,
  onSelect,
}: B2Trial2ProductDetailDialogProps) {
  ...
  function toggleHiddenDetail() {
    const nextOpen = !isHiddenDetailOpen;
    const baseLog = createLogBase();
    ...
    void trackAction({
      ...
      phase: "main",
      page: "product",
      type: nextOpen ? "open_hidden_detail" : "close_hidden_detail",
      meta: { implTrialId: getImplTrialId() },
      payload: { productId: product.id },
    });

    setIsHiddenDetailOpen(nextOpen);
  }
        ...
        <div className="h-full w-full overflow-hidden bg-white">
          ...
          <section className="h-[160px] border-b border-gray-200 px-[40px]">
            <div className="grid h-full grid-cols-[200px_1fr_180px]">
              ...
              {/* 右：B系では強調DPを置かない */}
              <div className="flex h-full items-center justify-center">
                <div className="h-[50px] w-[160px] overflow-hidden" aria-hidden="true" />
              </div>
            </div>
          </section>
          ...
          <section className="grid h-[190px] grid-cols-[1fr_1fr] gap-[60px] px-[40px]">
            <div className="h-full overflow-hidden border border-gray-300 bg-white p-[20px]">
              <div className="h-[30px]">
                <h3 className="text-[18px] font-bold text-gray-900">商品説明</h3>
              </div>
            ...
            <div className="h-full overflow-hidden border border-gray-300 bg-white p-[20px]">
              <div className="flex h-[30px] items-center justify-between gap-4">
                <h3 className="text-[18px] font-bold text-gray-900">仕様・補足</h3>

                {product.hiddenDetailsTitle && (
                  <button
                    type="button"
                    className="shrink-0 text-[14px] font-semibold text-blue-700 underline underline-offset-2"
                    onClick={toggleHiddenDetail}
                  >
                    {isHiddenDetailOpen ? "閉じる" : product.hiddenDetailsTitle}
                  </button>
                )}
              </div>
              ...
              <div className="h-[120px] overflow-hidden text-[15px] font-medium leading-[23px] text-gray-700">
                {product.hiddenDetailsTitle ? (
                  isHiddenDetailOpen ? (
                    <div>{renderInfoLines(product.specsAndNotes)}</div>
                  ) : (
          ...
          <section className="grid h-[190px] grid-cols-[1fr_1fr] gap-[60px] px-[40px]">
            ...
            <div className="h-full overflow-hidden border border-gray-300 bg-white p-[20px]">
              <div className="h-[30px]">
                <h3 className="text-[18px] font-bold text-gray-900">配送情報</h3>
              </div>
