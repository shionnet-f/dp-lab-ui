      ...
      <dialog id={dialogId} className="m-auto w-full max-w-4xl rounded-2xl p-0 backdrop:bg-black/30">
        <div className="rounded-2xl bg-white">
          ...
          <div className="space-y-5 px-6 py-6">
            ...
            <div className="pt-1">
              <Link href={`/trials/b2/trial1-5/checkout?productId=${product.id}&set=${set}`} className="inline-flex h-12 w-full items-center justify-center rounded-md bg-black px-5 text-sm font-medium text-white">
                この商品を選ぶ
              </Link>
            </div>
          </div>
    ...
    <article className="h-[136px] rounded-xl border border-gray-200 bg-white px-5 shadow-sm">
      <div className="grid h-full grid-cols-[112px_1fr_260px] items-center gap-5">
        ...
        <div className="grid grid-cols-2 gap-3 justify-self-end">
          ...
          <Link href={`/trials/b2/trial1-5/checkout?productId=${product.id}&set=${set}`} className="flex h-11 items-center justify-center rounded-md bg-black px-4 text-sm font-medium text-white">
            購入へ
          </Link>
        </div>
      </div>
...
export default function B2trial1_5ProductPage() {
    ...
    <main className="h-screen overflow-hidden bg-gray-50 px-8 py-8">
      <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        <section className="grid flex-1 gap-5">
          {trial1_5Data.products.map((product) => <ProductCard key={product.id} product={product} set={set} />)}
        </section>
      </div>
    </main>
  );
