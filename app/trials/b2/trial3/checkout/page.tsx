...
const confirmPath = getTrialPath("b2", "trial3", "confirm");
const productPath = getTrialPath("b2", "trial3", "product");
...
export default function CheckoutPageB2Trial3({ searchParams }: Props) {
  ...
  const selectedShippingMethod = trial3Data.shippingMethods.find((method) => method.id === shipping) ?? null;
  const selectedOptionItems = trial3Data.options.filter((option) => options.includes(option.id));
