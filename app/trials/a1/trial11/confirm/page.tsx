...
const highestShippingPrice = 800;
...
function getShippingDisplayText(priceYen: number) {
  const diff = highestShippingPrice - priceYen;
  if (diff === 0) return "¥0";
  return `-¥${yen(diff)}`;
}
...
export default async function ConfirmPageA1Trial11({ searchParams }: Props) {
        ...
        <div className="grid flex-1 grid-rows-[520px_120px] gap-6">
          <section className="grid h-full grid-cols-[1.5fr_1fr] gap-6">
            <div className="grid h-full grid-rows-[180px_110px_1fr] gap-6">
              <article className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm overflow-hidden">
              ...
              <article className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm overflow-hidden">
                ...
                <div className="flex items-center justify-between text-sm text-gray-700">
                  ...
                  <span>{getShippingDisplayText(shippingInfo.priceYen)}</span>
                </div>
              </article>
            ...
            <div className="grid h-full grid-rows-[1fr_auto] rounded-xl border border-gray-200 bg-white p-5 shadow-sm overflow-hidden">
              <div>
                <h2 className="mb-4 text-base font-semibold text-gray-900">
                  お支払い金額
                </h2>
              ...
              <div className="grid gap-3 pt-6">
                <Link
                  href={`/trials/a1/trial11/complete?${completeParams.toString()}`}
                ...
                <Link
                  href={`/trials/a1/trial11/checkout?${backParams.toString()}`}
