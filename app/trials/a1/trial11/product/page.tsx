...
function FreeShippingBadge({ visible }: { visible: boolean }) {
  if (!visible) return <div className="h-full w-full" aria-hidden="true" />;
  return (
    <div className="flex h-full items-center rounded-md bg-red-50 px-3 py-2 text-sm font-bold text-red-600">
      送料無料
    </div>
  );
}
            ...
            <div className="grid grid-rows-[160px_140px_120px_1fr] gap-5">
              <section className="overflow-hidden rounded-xl border-2 border-gray-300 p-4">
                <div className="flex h-full flex-col justify-start">
                  {showFreeShipping ? (
                    <div className="mb-3 rounded-md bg-red-50 px-3 py-2 text-sm font-bold text-red-600">
                      送料無料
                    </div>
                  ) : (
      ...
      <div className="grid h-full grid-rows-[128px_78px_30px_40px] gap-4">
        ...
        <div className="grid h-[78px] grid-rows-[1fr_auto_auto] overflow-hidden">
          ...
          <div className="h-5 overflow-hidden">
            {showFreeShipping ? (
              <div className="text-sm font-bold leading-5 text-red-600">
                送料無料
              </div>
            ) : null}
          </div>
