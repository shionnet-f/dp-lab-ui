...
export default function CheckoutPageA1Trial11({ searchParams }: Props) {
  ...
  const shippingPrice =
    shipping ? (trial11Data.shippingMethods.find((m) => m.id == shipping)?.priceYen ?? 0) : 0;
  ...
  return (
    <main className="h-screen overflow-hidden bg-gray-50 px-8 py-8">
      <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        <form
          action="/trials/a1/trial11/confirm"
          method="GET"
          ...
          <div className="space-y-10">
            <article className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="mb-4 rounded-md bg-gray-50 px-4 py-3 text-sm text-gray-600">
                通常配送は無料です。お急ぎ便・当日便は追加料金がかかります。
              </div>

              <div className="space-y-4 text-sm text-gray-700">
                {trial11Data.shippingMethods.map((method) => (
                  <label key={method.id} className="flex items-start gap-3 rounded-md border border-gray-200 px-4 py-3">
                    <input
                      type="radio"
                      name="shippingRadio"
            ...
            <article className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-4 text-sm text-gray-700">
                {trial11Data.options.map((option) => (
                  <label key={option.id} className="flex items-start gap-3 rounded-md border border-gray-200 px-4 py-3">
                    <input
                      type="checkbox"
          ...
          <div className="flex h-full flex-col rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-auto space-y-6 text-gray-900">
              <div className="space-y-3 pt-4">
                ...
                <Link
                  href={`/trials/a1/trial11/product?set=${set}`}
