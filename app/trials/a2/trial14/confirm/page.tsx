...
export default async function ConfirmPageA2Trial14({ searchParams }: Props) {
        ...
        <div className="grid flex-1 grid-rows-[152px_260px_176px_120px_88px] gap-6">
            ...
            <article className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              ...
              <div className="rounded-md border border-gray-100 bg-gray-50 px-4 py-3">
                ...
                <div className="mt-1 text-sm text-gray-500">
                  {shippingInfo.priceYen > 0 ? `¥${yen(shippingInfo.priceYen)}` : "選択されていません"}
                </div>
              </div>
            </article>
          ...
          <section className="rounded-xl border border-gray-200 bg-white px-6 py-5 shadow-sm">
            <div className="flex h-full flex-col gap-3">
              <Link href={`/trials/a2/trial14/complete?${completeParams.toString()}`} className="flex h-12 w-full items-center justify-center rounded-md bg-black px-5 text-sm font-medium text-white">購入を確定する</Link>

              <Link href={`/trials/a2/trial14/checkout?${backParams.toString()}`} className="flex h-12 w-full items-center justify-center rounded-md border border-gray-300 bg-white px-5 text-sm font-medium text-gray-700">戻る</Link>
            </div>
          </section>
