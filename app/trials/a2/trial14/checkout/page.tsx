...
function getPricePresentation(productId: string | undefined, priceYen: number) {
  const index = products6.findIndex((product) => product.id === productId);
  const isDiscounted = index >= 0 && discountFlags[index] === 1;
  const mapped = index >= 0 ? discountPriceMap[index] : null;

  if (!isDiscounted || !mapped) {
    return { isDiscounted: false, originalPriceYen: priceYen, displayPriceYen: priceYen };
  }

  return { isDiscounted: true, originalPriceYen: mapped.originalPriceYen, displayPriceYen: mapped.displayPriceYen };
}

function ProductPriceInline({ productId, priceYen }: { productId?: string; priceYen: number }) {
  const price = getPricePresentation(productId, priceYen);

  if (!price.isDiscounted) {
    return <>商品価格：¥{yen(priceYen)}</>;
  }
  ...
  return (
    <span className="inline-flex items-end gap-2">
      ...
      <span className="font-medium text-red-600">¥{yen(price.displayPriceYen)}</span>
    </span>
  );
}

function ProductPriceRow({ productId, priceYen }: { productId?: string; priceYen: number }) {
  const price = getPricePresentation(productId, priceYen);

  if (!price.isDiscounted) {
    return <span>¥{yen(priceYen)}</span>;
  }

  return (
    <span className="inline-flex items-end gap-2">
      <span className="text-sm text-gray-500 line-through">¥{yen(price.originalPriceYen)}</span>
      <span className="text-red-600">¥{yen(price.displayPriceYen)}</span>
    </span>
  );
}

export default function CheckoutPageA2Trial14({ searchParams }: Props) {
  ...
  return (
    <main className="h-screen overflow-hidden bg-gray-50 px-8 py-8">
      <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        <form
          action="/trials/a2/trial14/confirm"
          method="GET"
        ...
        >
            ...
            <div className="grid h-full grid-cols-[120px_1fr_auto] items-center gap-6">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-3 text-base text-gray-600">
                  <ProductPriceInline productId={selectedProduct.id} priceYen={selectedProduct.priceYen} />
                </div>
          ...
          <section className="grid h-full grid-cols-2 gap-5">
            <article className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <label className="flex items-start gap-3 rounded-md border border-gray-100 bg-gray-50 px-3 py-2">
                  <input type="radio" name="shippingRadio" value="standard" checked={shipping === "standard"} onChange={(e) => setShipping(e.target.value)} />
                  <div>
                ...
                <label className="flex items-start gap-3 rounded-md border border-gray-100 bg-gray-50 px-3 py-2">
                  <input type="radio" name="shippingRadio" value="express" checked={shipping === "express"} onChange={(e) => setShipping(e.target.value)} />
                  <div>
                ...
                <label className="flex items-start gap-3 rounded-md border border-gray-100 bg-gray-50 px-3 py-2">
                  <input type="radio" name="shippingRadio" value="scheduled" checked={shipping === "scheduled"} onChange={(e) => setShipping(e.target.value)} />
                  <div>
                    <div className="font-medium text-gray-700">日時指定便</div>
                    <div className="text-gray-500">受け取り日時を指定できます</div>
            ...
            <article className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <label className="flex items-start gap-3 rounded-md border border-gray-100 bg-gray-50 px-3 py-2">
                  <input type="checkbox" checked={options.includes("insurance")} onChange={() => toggleOption("insurance")} />
                  <div>
                    <div className="font-medium text-gray-700">配送補償オプション</div>
                    <div className="text-gray-500">破損・紛失時の補償を追加します</div>
                    <div className="text-gray-500">+¥{yen(300)}</div>
                  </div>
                </label>

                <label className="flex items-start gap-3 rounded-md border border-gray-100 bg-gray-50 px-3 py-2">
                  <input type="checkbox" checked={options.includes("gift")} onChange={() => toggleOption("gift")} />
                  <div>
                    <div className="font-medium text-gray-700">ギフト包装</div>
                    <div className="text-gray-500">プレゼント用に包装します</div>
            ...
            <div className="grid h-full grid-cols-[1fr_auto] items-center gap-8">
              <div className="space-y-4 text-sm text-gray-900">
                <div className="flex items-center justify-between">
                  <span>商品価格</span>
                  <ProductPriceRow productId={selectedProduct.id} priceYen={selectedProduct.priceYen} />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Link href="/trials/a2/trial14/product" className="flex h-11 items-center justify-center rounded-md border border-gray-300 px-5 text-sm font-medium text-gray-700">商品一覧へ戻る</Link>

                <button type="submit" className="h-11 rounded-md bg-black px-5 text-sm font-medium text-white">次へ進む</button>
              </div>
            </div>
          </section>
        </form>
