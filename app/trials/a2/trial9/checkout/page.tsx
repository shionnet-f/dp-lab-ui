...
import { getProductById, trial9Data } from "../data";

const confirmPath = getTrialPath("a2", "trial9", "confirm");
const productPath = getTrialPath("a2", "trial9", "product");
...
export default function CheckoutPageA2Trial9({ searchParams }: Props) {
  ...
  const selectedShippingMethod =
    trial9Data.shippingMethods.find((method) => method.id === shipping) ?? null;

  const selectedOptionItems = trial9Data.options.filter((option) =>
        ...
        >
          ...
          <section className="grid h-[438px] w-[1160px] grid-cols-[550px_550px] gap-[60px] overflow-hidden">
            <ShippingMethodSection
              shippingMethods={trial9Data.shippingMethods}
              selectedShipping={shipping}
              onChangeShipping={setShipping}
            />

            <OptionSection
              options={trial9Data.options}
