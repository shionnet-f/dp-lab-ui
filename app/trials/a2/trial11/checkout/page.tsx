...
const confirmPath = getTrialPath("a2", "trial11", "confirm");
const productPath = getTrialPath("a2", "trial11", "product");
...
export default function CheckoutPageA2Trial11({ searchParams }: Props) {
  ...
  const selectedShippingMethod =
    trial11Data.shippingMethods.find((method) => method.id === shipping) ?? null;

  const selectedOptionItems = trial11Data.options.filter((option) =>
        ...
        >
          ...
          <section className="grid h-[438px] w-[1160px] grid-cols-[550px_550px] gap-[60px] overflow-hidden">
            <ShippingMethodSection
              shippingMethods={trial11Data.shippingMethods}
              selectedShipping={shipping}
              onChangeShipping={setShipping}
            />

            <OptionSection
              options={trial11Data.options}
