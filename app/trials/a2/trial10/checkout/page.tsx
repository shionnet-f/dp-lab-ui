...
export default function CheckoutPageA2Trial10({ searchParams }: Props) {
  const sp = use(searchParams);

  const productId = sp?.productId;
  const initialShipping = sp?.shipping ?? "";
  const initialInsuranceReject = sp?.insuranceReject === "1";
  const initialGiftReject = sp?.giftReject === "1";

  const selectedProduct =
    products6.find((product) => product.id === productId) ?? products6[0];

  const [shipping, setShipping] = useState<string>(initialShipping);
  const [insuranceReject, setInsuranceReject] = useState<boolean>(
    initialInsuranceReject,
  );
  const [giftReject, setGiftReject] = useState<boolean>(initialGiftReject);
  ...
  const optionTotal = useMemo(() => {
    let total = 0;
    if (!insuranceReject) total += optionPrices.insurance;
    if (!giftReject) total += optionPrices.gift;
    return total;
  }, [insuranceReject, giftReject]);
  ...
  return (
    <main className="h-screen overflow-hidden bg-gray-50 px-8 py-8">
      <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        <form
          action="/trials/a2/trial10/confirm"
          method="GET"
        ...
        >
          ...
          <input
            ...
            value={insuranceReject ? "1" : "0"}
          />
          <input
            type="hidden"
            name="giftReject"
            value={giftReject ? "1" : "0"}
          />
            ...
            <article className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-2 text-sm text-gray-600">
                <label className="flex items-start gap-3 rounded-md border border-gray-100 bg-gray-50 px-3 py-2">
                  <input
                    type="checkbox"
                    checked={insuranceReject}
                    onChange={(e) => setInsuranceReject(e.target.checked)}
                  />
                  <div>
                ...
                <label className="flex items-start gap-3 rounded-md border border-gray-100 bg-gray-50 px-3 py-2">
                  <input
                    type="checkbox"
                    checked={giftReject}
                    onChange={(e) => setGiftReject(e.target.checked)}
                  />
                  <div>
                    <div className="font-medium text-gray-700">
                      ギフト包装を希望しない
          ...
          <section className="rounded-xl border border-gray-200 bg-white px-6 py-6 shadow-sm">
            <div className="grid h-full grid-cols-[1fr_auto] items-center gap-8">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Link
                  href="/trials/a2/trial10/product"
