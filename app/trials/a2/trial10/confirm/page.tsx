...
export default async function ConfirmPageA2Trial10({ searchParams }: Props) {
  ...
  const selectedOptions = [
    insuranceReject !== "1"
      ? { label: "配送補償オプション", priceYen: 300 }
      : null,
    giftReject !== "1" ? { label: "ギフト包装", priceYen: 200 } : null,
  ].filter(
    (item): item is { label: string; priceYen: number } => item !== null,
  );
        ...
        <div className="grid flex-1 grid-rows-[152px_260px_176px_120px_88px] gap-6">
          ...
          <section className="rounded-xl border border-gray-200 bg-white px-6 py-5 shadow-sm">
            <div className="flex h-full flex-col gap-3">
              <Link
                href={`/trials/a2/trial10/complete?${completeParams.toString()}`}
              ...
              <Link
                href={`/trials/a2/trial10/checkout?${backParams.toString()}`}
