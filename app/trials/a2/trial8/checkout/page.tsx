...
function RelativeShippingDisplay({
  priceYen,
  relativeDeltaYen,
}: {
  priceYen: number;
  ...
  const delta = relativeDeltaYen ?? 0;
  const deltaText = delta === 0 ? "0" : `${delta > 0 ? "+" : ""}${yen(delta)}`;
  const deltaClass =
    delta < 0
      ? "text-red-600"
      : delta > 0
  ...
  return (
    <div className="mt-1 flex items-center gap-3 text-sm">
      ...
      <span className="text-gray-500">(実際の送料: ¥{yen(priceYen)})</span>
    </div>
  );
}
...
export default function CheckoutPageA2Trial8({ searchParams }: Props) {
  ...
  const baseShipping =
    trial8Data.shippingMethods.find((method) => method.id === "express") ??
    trial8Data.shippingMethods[0]

  const formatRelative = (priceYen: number) => {
    const diff = priceYen - baseShipping.priceYen
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-gray-500">
                          {formatRelative(method.priceYen)}
                        </span>
                      </div>
                    </div>
                  </label>
