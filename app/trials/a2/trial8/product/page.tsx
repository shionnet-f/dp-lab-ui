...
function AwardInfo({ show, awardText }: AwardInfoProps) {
  return (
    <div className="h-11 w-[160px] overflow-hidden">
      {show ? (
        <div className="flex h-full w-full items-center rounded-md bg-violet-50 px-3 text-sm text-violet-700">
          <p className="line-clamp-2 font-medium leading-5">{awardText}</p>
        </div>
      ) : (
      ...
      >
        <div className="rounded-2xl bg-white">
          ...
          <div className="space-y-5 px-6 py-6">
            <section className="rounded-xl border border-gray-200 p-5">
              <div className="grid grid-cols-[160px_1fr] items-center gap-5">
                ...
                <div className="min-w-0">
                  {showAward ? (
                    <div className="mb-3 w-[260px] rounded-md bg-violet-50 px-3 py-2 text-sm font-medium text-violet-700">
                      {awardText}
                    </div>
                  ) : (
            ...
            <div className="pt-1">
              <Link
                href={`/trials/a2/trial8/checkout?productId=${product.id}`}
...
export default function ProductPageA2Trial8() {
  const awardTexts = [
    "モンドセレクション受賞商品",
    "人気商品アワード受賞",
    "レビュー高評価賞 受賞",
