...
const confirmPath = getTrialPath("a2", "trial6", "confirm");
const productPath = getTrialPath("a2", "trial6", "product");
...
export default function CheckoutPageA2Trial6({ searchParams }: Props) {
  ...
  const selectedShippingMethod =
    trial6Data.shippingMethods.find((method) => method.id === shipping) ?? null;

  const selectedOptionItems = trial6Data.options.filter((option) =>
        ...
        >
          ...
          <section className="grid h-[438px] w-[1160px] grid-cols-[550px_550px] gap-[60px] overflow-hidden">
            <ShippingMethodSection
              shippingMethods={trial6Data.shippingMethods}
              selectedShipping={shipping}
              onChangeShipping={setShipping}
            />

            <OptionSection
              options={trial6Data.options}
