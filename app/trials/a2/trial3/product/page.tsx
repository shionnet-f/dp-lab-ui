...
function renderDpBadge(product: Trial3Product) {
  if (!product.dpDisplay) return null;
  return (
    <div className="shrink-0 rounded-md border border-amber-200 bg-amber-50 px-3 py-1 text-sm font-semibold text-amber-700">
      <span>{product.dpDisplay.label}</span>
      {product.dpDisplay.subLabel ? <span className="ml-1 text-xs font-medium text-gray-500">{product.dpDisplay.subLabel}</span> : null}
    </div>
  );
}
      ...
      <dialog id={dialogId} className="m-auto w-full max-w-4xl rounded-2xl p-0 backdrop:bg-black/30">
        <div className="rounded-2xl bg-white">
          ...
          <div className="space-y-5 px-6 py-6">
              ...
              <div className="grid grid-cols-[160px_1fr] items-center gap-5">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-5">
                    <div className="text-xl font-semibold text-gray-900">¥{yen(product.priceYen)}</div>
                    {renderDpBadge(product)}
                  </div>
                </div>
              </div>
            </section>
            ...
            <div className="pt-1">
              <Link href={`/trials/a2/trial3/checkout?productId=${product.id}`} className="inline-flex h-12 w-full items-center justify-center rounded-md bg-black px-5 text-sm font-medium text-white">
                この商品を選ぶ
              </Link>
            </div>
          </div>
...
function ProductCard({ product }: { product: Trial3Product }) {
  return (
    <article className="h-[136px] rounded-xl border border-gray-200 bg-white px-5 shadow-sm">
      <div className="grid h-full grid-cols-[112px_1fr_260px] items-center gap-5">
        ...
        <div className="grid grid-cols-2 gap-3 justify-self-end">
          ...
          <Link href={`/trials/a2/trial3/checkout?productId=${product.id}`} className="flex h-11 items-center justify-center rounded-md bg-black px-4 text-sm font-medium text-white">
            購入へ
          </Link>
        </div>
      </div>
    ...
    <main className="h-screen overflow-hidden bg-gray-50 px-8 py-8">
      <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        <section className="grid flex-1 gap-5">
          {trial3Data.products.map((product) => <ProductCard key={product.id} product={product} />)}
        </section>
      </div>
    </main>
  );
